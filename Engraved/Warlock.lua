...
function Warlock:UpdateSoulShards()
	local resource = UnitPower("player", SPELL_POWER_SOUL_SHARDS, true) / UnitPowerDisplayMod(SPELL_POWER_SOUL_SHARDS)
	local shards, partial = math.modf(resource)
	local alpha = resource - shards;
	...
	local adjust = 1;
	if ( partial ~= 0 ) then
		self.Runes[shards + 1].fill:SetAlpha(alpha);
		adjust = 2;
	end
	for i = shards + adjust, 5 do
